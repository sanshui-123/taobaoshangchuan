...
async function createFolderWithoutSearchPanel() {
  const productId = 'C25291153';

  const log = (message, type = 'info') => {
    const timestamp = new Date().toLocaleTimeString();
    const prefix = type === 'error' ? '❌' : type === 'success' ? '✅' : type === 'warning' ? '⚠️' : '🔧';
    console.log(`${timestamp} FixedCreate: ${prefix} ${message}`);
  };
  ...
  try {
    log(`开始修正版文件夹创建流程，目标ID: ${productId}`);
    ...
    // 步骤2：点击弹窗内的"所属上级文件夹"输入框
    log('=== 步骤2：点击弹窗内的所属上级文件夹输入框 ===');
    ...
    log(`❌ 修正版文件夹创建失败: ${error.message}`, 'error');
    throw error;
  }
}

// 执行创建
createFolderWithoutSearchPanel()
  .then((success) => {
    if (success) {
